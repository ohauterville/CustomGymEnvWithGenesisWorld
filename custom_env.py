...
class CustomEnv(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        obs_low = np.concatenate([joint_pos_low, joint_vel_low, ee_pos_low, target_pos_low, dist_low]).astype(np.float32)
        obs_high = np.concatenate([joint_pos_high, joint_vel_high, ee_pos_high, target_pos_high, dist_high]).astype(np.float32)
